...
export const CopyCoordinate = ({ mapRef }) => {
  ...
  const addMarkerToMap = () => {
    ...
    setNewMarker(
      L.marker(
        [
          (mapRef.current.getBounds()._northEast.lat +
            mapRef.current.getBounds()._southWest.lat) /
            2,
          (mapRef.current.getBounds()._northEast.lng +
            mapRef.current.getBounds()._southWest.lng) /
            2
        ],
        { icon: markerIcon, draggable: true }
      )
        .addTo(mapRef.current)
        .on('dragend', setCoordinatesInState)
    )
  }

  const setCoordinatesInState = (e) =>
    setCoordinates([e.target._latlng.lat, e.target._latlng.lng])
  ...
  return (
    <div style={{ paddingTop: 10 }}>
      ...
      {markerOnMap && (
        <button className={'button'} onClick={copyCoordinatesInBuffer}>
